...
export default async function handler(req, res) {
  ...
  // action will be an array from catch-all route, e.g., ['badge'] or ['checkin']
  ...
  const validActions = ['badge', 'checkin', 'drink']
  if (!validActions.includes(actionType)) {
    return res.status(404).json({ error: `Unknown action: ${actionType}` })
  }
